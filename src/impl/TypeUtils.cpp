...
CreateMilvusFieldData(const milvus::proto::schema::FieldData& field_data) {
    ...
    switch (field_type) {
        ...
        case proto::schema::DataType::Float16Vector:
            ...
            return std::make_shared<Float16VecFieldData>(
                name,
                BuildFieldDataVectors<std::string>(field_data.vectors().dim(), field_data.vectors().float16_vector()));

        case proto::schema::DataType::BFloat16Vector:
            // 1 bfloat16 element = 2 bytes
            return std::make_shared<BFloat16VecFieldData>(
                name,
                BuildFieldDataVectors<std::string>(field_data.vectors().dim(), field_data.vectors().bfloat16_vector()));
