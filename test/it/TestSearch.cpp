...
TEST_F(MilvusMockedTest, SearchFooWithTimeoutExpired) {
    ...
    auto status = DoSearchVectors<milvus::FloatVecFieldData>(service_, client_, float_vectors, search_results, 1000, 500);
    auto duration =
...
TEST_F(MilvusMockedTest, SearchFooWithTimeoutOk) {
    ...
    auto t0 = std::chrono::system_clock::now();
    auto status = DoSearchVectors<milvus::FloatVecFieldData>(service_, client_, float_vectors, search_results, 500, 1000);
    auto duration =
        std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - t0).count();
